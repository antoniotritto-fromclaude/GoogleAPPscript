...
const insertContatto = db.prepare(`
  ...
  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, date('now', '-' || abs(random() % 180) || ' days'), date('now', '-' || abs(random() % 30) || ' days'))
`);
...
const probabilita = { 'Lead': 10, 'Contatto': 20, 'Qualificato': 40, 'Proposta Inviata': 60, 'Negoziazione': 75, 'Contratto Inviato': 85, 'Contratto Firmato': 95, 'Cliente Attivo': 100 };
...
const insertContratto = db.prepare(`
  ...
  VALUES ('CTR-2025-' || printf('%03d', ?), ?, ?, ?, ?, ?, ?, ?, ?, date('now', '-' || abs(random() % 180) || ' days'),
    CASE WHEN ? IN ('Attivo', 'Firmato') THEN date('now', '-' || abs(random() % 90) || ' days') ELSE NULL END,
    CASE WHEN ? = 'Attivo' THEN date('now', '-' || abs(random() % 60) || ' days') ELSE NULL END,
    12, date('now', '+' || (365 - abs(random() % 180)) || ' days'), 1)
`);
contratti.forEach(row => insertContratto.run(row[0], row[0], row[1], row[2], row[3], row[4], row[5], row[6], row[7], row[7], row[7]));
...
const insertChiamata = db.prepare(`
  ...
  VALUES ('CALL-' || printf('%04d', ?), ?, ?, ?, ?, ?, ?, ?, ?, date('now', '-' || abs(random() % 30) || ' days'),
    CASE WHEN ? IN ('Richiamare', 'Info Richieste', 'Interessato') THEN date('now', '+' || abs(random() % 14) || ' days') ELSE NULL END,
    CASE WHEN ? = 'Completato' THEN 1 ELSE 0 END)
`);
chiamate.forEach((row, i) => insertChiamata.run(i+1, ...row, row[4], row[4]));
...
const insertTimeline = db.prepare(`
  ...
  VALUES (?, ?, ?, ?, ?, ?, ?, ?, CASE WHEN ? IS NOT NULL AND ? IS NOT NULL THEN 1 ELSE 0 END, datetime('now', '-' || abs(random() % 90) || ' days'))
`);
timeline.forEach(row => insertTimeline.run(...row, row[6], row[7]));
